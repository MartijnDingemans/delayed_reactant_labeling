...
class VisualizeModel:
    ...
    def plot_grouped_by(self,
        ...
        **fig_kwargs
        ...
        for n, arg in enumerate(args):  # contain the same input
            ...
            data.append(arg)
            index.append(arg.name if arg.name is not None else n + 1)
        ...
        for ax, (group_key, selected_x) in zip(axs, key_hits.iterrows()):
            bars = data.loc[:, selected_x].T.plot.bar(ax=ax)
    ...
    def plot_enantiomer_ratio(self,
        ...
        def analyze_data(data: pd.DataFrame, marker, descr):
            for group_n, group in enumerate(group_by):
                ...
                ratio_of_index = []  # index of hits for each element in ratio_of, corrected for multiple values
                ...
                for element_n, el_ind in enumerate(ratio_of_index):
                    ...
                    value = grouped_data.iloc[-last_N:, el_ind].divide(grouped_data_sum, axis=0).mean()
                    value = value.item() if isinstance(value, pd.Series) else value
                    scatter_point = ax.scatter(group_n,
                                               value,
                                               marker=marker,
                                               label=label,
                                               color=f'C{element_n}',
                                               alpha=0.7, s=100)
                    # Store text for adjustment
                    text = ax.text(group_n, value, f'{value:.2f}', ha='center', va='bottom')
                    texts.append(text)
    ...
    def plot_rate_sensitivity(self,
        ...
        **requires a single model**
        ...
        thresh
            The threshold (defined from the minimum error) for the confidence interval (i.e. 1.2 is confidence interval of 20%)
        **fig_kwargs
            Additional keyword arguments that are passed on to plt.subplots().
        """
        if file_name is None:
        ...
        confidence_intervals = {}  # Store CI per parameter

        for col, param in enumerate(ticks.index):  # `ticks.index` gives param names
            param_errors = errors[:, col]
            min_error = np.nanmin(param_errors)
            threshold = thresh * min_error

            valid_xs = x_value[param_errors <= threshold]
            if len(valid_xs) > 0:
                ci_lower = valid_xs.min()
                ci_upper = valid_xs.max()
                confidence_intervals[param] = (ci_lower, ci_upper)
            else:
                confidence_intervals[param] = (np.nan, np.nan)

        for param, (low, high) in confidence_intervals.items():
            print(f"{param}: CI = [{low:.4g}, {high:.4g}]")

        # Plot confidence intervals
        fig_ci, ax_ci = plt.subplots(figsize=(8, 6))

        params = list(confidence_intervals.keys())
        cis = np.array([confidence_intervals[p] for p in params])
        ...
        # Plot error bars
        ax_ci.errorbar(x_pos, y_mid, yerr=y_err, fmt='none', ecolor='r', capsize=5)

        # Add labels: lower bound, upper bound, and midpoint
        for i, param in enumerate(params):
            low, high = confidence_intervals[param]
            if not np.isnan(low) and not np.isnan(high):
                mid = (low + high) / 2

                # Lower bound
                ax_ci.text(i, low * 0.95, f"{low:.3g}",
                           ha='center', va='top', fontsize=9, color='blue')

                # Upper bound
                ax_ci.text(i, high * 1.05, f"{high:.3g}",
                           ha='center', va='bottom', fontsize=9, color='blue')

        # Axis formatting
        ax_ci.set_xticks(x_pos)
        ax_ci.set_xticklabels(params, fontsize=12, rotation=45)
        ax_ci.set_yscale('log')
        ax_ci.set_xlabel("Rate constants", fontsize=14)
        ax_ci.set_ylabel(None, fontsize=14)
        ax_ci.set_title("Confidence Intervals", fontsize=16)

        plt.tight_layout()
        self.save_image(fig_ci, file_name + "_ci")
        plt.close(fig_ci)
        ...
        ax.set_xticklabels(ticks.index, fontsize=15)
        ...
        cbar = fig.colorbar(im, cax=cax, label="error")
    ...
    def plot_error_all_runs(self,
        ...
        **fig_kwargs
        ...
        ax.set_xlabel('Solution nr. (sorted by error)', fontsize=20)
    ...
    def plot_x_all_runs(self, index: slice, *,
        ...
        **requires multiple model**
        ...
        index
            ...
            index=slice(1, 5) would plot the results of runs 1 up to and including 4,
        ...
        **fig_kwargs
        ...
        # Step 1: Slice the rows based on the index (apply slicing here)
        sliced_df = self.models.all_optimal_x.iloc[index, :]

        # Step 2: Filter columns (parameters) after slicing rows
        df = sliced_df.loc[:, ~self.hide_params]

        # Step 3: Create the boxplot
        fig, ax = plt.subplots(**fig_kwargs)
        ax.boxplot(df)
        ax.set_title(None)
        ax.set_xticks(1 + np.arange(len(df.columns)))  # X-ticks for parameters
        ax.set_xticklabels(df.columns, rotation=90)  # Parameter names as x-axis labels
